...
const techGroups: Array<{ title: string; items: TechItem[] }> = [
  {
    ...
    items: [
      ...
      { name: "Terraform", image: publicPath("/images/tech/terraform.svg") },
      { name: "Ansible", image: publicPath("/images/tech/ansible.svg") },
    ],
  },
  {
    title: "Container & Operations",
    items: [
      { name: "Kubernetes", image: publicPath("/images/tech/kubernetes.svg") },
  ...
  },
  {
    ...
    items: [
      ...
      { name: "Shell", image: publicPath("/images/tech/shell.svg"), iconClassName: "h-16 w-16" },
    ],
  },
];
...
function TechIcon({ item }: { item: TechItem }) {
  return (
    <div className="group relative flex flex-col items-center">
      ...
      <span className="mt-2 whitespace-nowrap text-center text-[11px] leading-4 text-muted-foreground">
        {item.name}
      </span>
    </div>
  );
  ...
  return (
    <div className="space-y-24">
      ...
      <section>
        ...
        <div className="mt-2 grid gap-x-16 gap-y-10 lg:grid-cols-2">
      ...
      <section className="pb-16">
        ...
        <div className="mt-2 grid gap-4 sm:grid-cols-2">
          ...
          <Link
            href="mailto:imjwoo15@gmail.com"
            className="flex items-center gap-3 rounded-lg border border-border bg-card p-4 transition-colors hover:bg-accent/10"
          >
            <Mail className="h-7 w-7 shrink-0 text-foreground" />
            <span className="min-w-0 flex-1">
              <span className="block text-sm font-medium">Email</span>
              <span className="mt-1 block truncate text-sm text-muted-foreground">imjwoo15@gmail.com</span>
            </span>
            <ExternalLink className="h-4 w-4 shrink-0 text-muted-foreground" />
          </Link>
        </div>
